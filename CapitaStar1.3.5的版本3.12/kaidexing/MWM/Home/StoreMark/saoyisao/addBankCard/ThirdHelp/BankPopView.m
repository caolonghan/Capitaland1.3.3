    ...
    [HttpClient requestWithMethod:@"POST" path:[Util makeRequestUrl:@"unionpay/UnionpayBindCard" tp:@"get_member_card_list"] parameters:diction target:self success:^(NSDictionary *dic) {
        NSLog(@"%@",dic);
         dispatch_async(dispatch_get_main_queue(), ^{
             ...
             NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:@"",@"iamge" ,@"使用新卡付款",@"card_name",nil];
             [arr addObject:dic];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        if (indexPath.row ==_bankArr.count-1) {
            cell.tipImageView.image = [UIImage imageNamed:@"Rectangle 9"];
            cell.bankLabel.text = _bankArr[indexPath.row][@"card_name"];
            cell.bankImageView.image = [UIImage imageNamed:@""];
             cell.tipImageView.image = [UIImage imageNamed:@"Rectangle 9 Copy"];
        }else{
        cell.bankImageView.image = [UIImage imageNamed:@""];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == _bankArr.count-1) {
       [self.delegate addBankCard];
        ...
        [self.delegate changeBankCard:bankName ides: _bankArr[indexPath.row][@"ides"]];
    }
}
