...
@interface CardInfoViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UITextFieldDelegate,carInfoViewDelegate>
...
- (void)clickToNext
{
   ...
   //借记卡
    if ([_cardType isEqualToString:@"01"]) {
        ...
        _cardHolderName = _InfoArray[0];
        _cardHolderId = _InfoArray[1];
        ...
        _cardHolderName = _InfoArray[0];
        _cardHolderId = _InfoArray[1];
        _expiredMonth = [_InfoArray[2] substringToIndex:1];
        _expiredYear =[_InfoArray[2] substringFromIndex:2];
        _CVN2 = _InfoArray[3];
        _cardHolderPhone = _InfoArray[4];
    }
    ...
    telVc.certifTp = @"01";
    telVc.customerInfo = _customerInfo;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        ...
        if (indexPath.row ==0) {
            tagLabel.text = @"持卡人";
            titleField.placeholder = @"持卡人姓名";
            
        }else if (indexPath.row ==1){
          tagLabel.text = @"身份证";
          titleField.placeholder = @"请输入证件号码";
        }else{
            tagLabel.text = @"手机号";
