...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        ...
        cell.timeLabel.text = billDic[@"add_time"];
         ...
         cell.payLabel.text = [NSString stringWithFormat:@"支出%@  收入0",billDic[@"origTxnAmt"]];
        ...
        cell.payStyleLabel.text = billDic[@"statusName"];
        cell.payStyleLabel.textColor = [UIColor colorWithRed:0 green:135/255.0 blue:140/255.0 alpha:1];
        if ([billDic[@"status"] integerValue] ==0) {
            cell.payLabel.text = @"支出0  收入0";
            cell.moneyLabel.text =@"0";
        }else if ([billDic[@"status"] integerValue] ==1||[billDic[@"status"] integerValue] ==2){
            cell.payLabel.text = [NSString stringWithFormat:@"支出%@  收入0",billDic[@"origTxnAmt"]];
            cell.moneyLabel.text =[NSString stringWithFormat:@"-%@",billDic[@"origTxnAmt"]];
        }else
        {
            cell.payLabel.text = [NSString stringWithFormat:@"支出0  收入%@",billDic[@"origTxnAmt"]];
            cell.moneyLabel.text =[NSString stringWithFormat:@"+%@",billDic[@"origTxnAmt"]];
...
{
    if (tableView.mj_footer.hidden == YES&&section ==_dataArray.count-1) {
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, WIN_WIDTH, 44)];
        label.text = @"没有更多数据了";
