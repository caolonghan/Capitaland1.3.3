...
- (void)viewDidLoad
{
    ...
    activityVc = [[ActivityController alloc]init];
    // activityVc.buttomH=@"0";
    activityVc.isType=@"1";
    ...
    MyNBTabButton *activityBtn  = [[MyNBTabButton alloc] initWithIcon:[UIImage imageNamed:@"newActivitybarblack"]];
    activityBtn.highlightedIcon = [UIImage imageNamed:@"newActivitybar"];
    activityBtn.viewController  = activityVc;
    ...
    NSArray *arr = [NSArray arrayWithObjects:homeBtn,activityBtn,msgBtn,intBtn,mineBtn,nil];
...
-(void)locationTouch:(UIButton*)sender{
    ...
    if (sender.tag == 0) {
        ...
        [locationCueView removeFromSuperview];
        [homeVc.functionView removeFromSuperview];
        homeVc.functionView = nil;
        [homeVc loadAD];
        [homeVc loadOrderListData];
        [homeVc loadData];
    }else
    {
        [locationCueView removeFromSuperview];
