...
- (void)viewDidLoad {
    ...
    UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(30,20 , 100, 44)];
    titleLabel.text = @"活动";
    [self.leftBarItemView addSubview:titleLabel];
...
{
    if (_isPush) {
        [self redefineBackBtn:[UIImage imageNamed:@"newback"] :CGRectMake(0, 20, 44,44)];
    }else{
...
-(void)createRootView{
    self.hiddenView=[[UIView alloc]initWithFrame:CGRectMake(0, NAV_HEIGHT+20, SCREEN_FRAME.size.width, SCREEN_FRAME.size.height - NAV_HEIGHT-20)];
    self.hiddenView.backgroundColor=UIColorFromRGB(0xf2f2f2);
    
    headView = [[UIView alloc]initWithFrame:CGRectMake(0,0,WIN_WIDTH,M_WIDTH(43))];
...
//    [headView addSubview:segment];
    NSArray *segmentArr = [NSArray arrayWithObjects:@"商户活动",@"商场活动", nil];
    for (NSInteger i=0; i<2; i++) {
        UIButton *segmentBtn = [[UIButton alloc]initWithFrame:CGRectMake(i*WIN_WIDTH/4, 0, WIN_WIDTH/4, M_WIDTH(42))];
        segmentBtn.tag = 2000+i;
        [segmentBtn addTarget:self action:@selector(change:) forControlEvents:UIControlEventTouchUpInside];
        [segmentBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [segmentBtn setTitleColor:APP_BTN_COLOR forState:UIControlStateSelected];
        [segmentBtn setTitle:segmentArr[i] forState:UIControlStateNormal];
        segmentBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        [headView addSubview:segmentBtn];
        if (i==0) {
            segmentBtn.selected = YES;
        }
    }
    tip = [[UILabel alloc]initWithFrame:CGRectMake(10, M_WIDTH(42), WIN_WIDTH/4-20, 1)];
    [headView addSubview:tip];
    tip.backgroundColor = APP_BTN_COLOR;
    
    [self.hiddenView addSubview:headView];
    [self.view addSubview:self.hiddenView];
}

-(void)change:(UIButton *)Seg{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        tip.frame = CGRectMake((Seg.tag-2000)*WIN_WIDTH/4+10,M_WIDTH(42), WIN_WIDTH/4-20, 1);
    }];
    switch (Seg.tag-2000) {
